...
function initAjaxFilters() {
	jQuery('.ajax-filters-container').formAjaxLoader({
		form: 'form.filters-block',
		newHolderSelector: '.ajax-filter-holder',
		busyClass: 'is-busy',
		preAppendClass: 'new-item',
		changeMode: true,
		pagerLink: '.pagination a',
	}).on('FormAjaxLoader/loaded', function() {
	...
	jQuery('.filters-block').each(function() {
		...
		var categoriesForm = holder.find('.main-filters');
		var tags = holder.find('.secondary-filters .form-group');

		function filterTags() {
			var activeCategory = categoriesForm.find('[type="radio"]:checked');
			...
			if(activeCategory.length) {
				...
				if (activeCategory.val() == '*') {
					...
					tags.filter('.' + activeCategory.val()).show();
				}
			}
		}

		categoriesForm.on('change', filterTags);
		filterTags();
	});
}
...
function initSlickCarousel() {
	jQuery('.slider-holder').each(function() {
		var holder = jQuery(this);
		var slickSlider = holder.find('.main-slider');
	...
	// slider-secondary
	jQuery('.slider-secondary').each(function() {
		var holder = jQuery(this);
		var mainSlider = holder.find('.main-slider');
		var navSlider = holder.find('.nav-slick');

		if (navSlider.children().length < 5) {
			navSlider.addClass('no-transform');
		}
...
function initCustomHover() {
	jQuery('.accordion-item').touchHover({
		onHover: function() {
			jQuery(this.context).parent().addClass('hovered');
		},
		onLeave: function() {
			jQuery(this.context).parent().removeClass('hovered');
		}
	});
}
		...
		attachEvents: function() {
			...
			if(isTouchDevice || isWinPhoneDevice) {
				...
				this.toggleOn = function(e) {
					...
					var toggleOff = function(e) {
						...
						if (!target.is(element) && !target.closest(element).length) {
							...
							element.off('click', self.preventHandler);
					...
					if(!element.hasClass(self.options.hoverClass)) {
						setTimeout(function() {
							...
							element.one('click', self.preventHandler);
				...
				this.toggleOn = function(e) {
					var element = $(this);
					element.addClass(self.options.hoverClass);
					self.context.on(eventOff, self.options.selector, self.toggleOff);
					if(self.options.onHover) self.options.onHover(element);
				};
				this.toggleOff = function(e) {
					var element = $(this);
					element.removeClass(self.options.hoverClass);
					self.context.off(eventOff, self.options.selector, self.toggleOff);
					if(self.options.onLeave) self.options.onLeave(element);
				};
			}

			this.context.on(eventOn, this.options.selector, self.toggleOn);
		}
	};

	$.fn.touchHover = function(options) {
		return this.each(function() {
			var params = $.extend({}, options, {
					context: this
				}),
				instance = new TouchHover(params);
			$.data(this, 'TouchHover', instance);
		});
	};
}(jQuery));
